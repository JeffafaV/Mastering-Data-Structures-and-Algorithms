...
{
	// x will have the value of the node behind p
	int x = -65536; // default value when behind the first node
	
	while (p != NULL) // p is pointing to a node and not null
	{
		// if x is ever greater then it means the linked list is not sorted
		if (x > p->data)
		{
			return 0; // false
		}
		x = p->data; // x will have the current node's value
